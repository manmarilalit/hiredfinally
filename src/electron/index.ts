...
function installOllamaWindows(): Promise<void> {
    return new Promise((resolve, reject) => {
        ...
        wg.on("close", (c: number) =>
            c === 0
                ? resolve()
                : installOllamaWindowsDirect().then(resolve).catch(reject),
        );
        wg.on("error", () =>
            installOllamaWindowsDirect().then(resolve).catch(reject),
        );
...
async function installOllamaWindowsDirect(): Promise<void> {
    const os = require("os"),
        tmp = path.join(os.tmpdir(), "OllamaSetup.exe");
    ...
    await new Promise<void>((resolve, reject) => {
        ...
        i.on("close", (c: number) =>
            c === 0 ? resolve() : reject(new Error("code:" + c)),
        );
        i.on("error", reject);
    });
}
function installOllamaMac(): Promise<void> {
    return new Promise((resolve, reject) => {
        const b = spawn("brew", ["install", "ollama"], {
            stdio: "pipe",
            shell: true,
        });
        b.on("close", (c: number) =>
            c === 0
                ? resolve()
                : installOllamaMacDirect().then(resolve).catch(reject),
        );
...
async function installOllamaMacDirect(): Promise<void> {
    const os = require("os"),
        tmp = path.join(os.tmpdir(), "Ollama.dmg");
    mainWindow?.webContents.send("ollama-status", { status: "downloading" });
...
function downloadFile(url: string, dest: string): Promise<void> {
    return new Promise((resolve, reject) => {
        const https = require("https"),
            fsSync = require("fs"),
            file = fsSync.createWriteStream(dest);
        https
            .get(url, (res: any) => {
                res.pipe(file);
                file.on("finish", () => {
...
app.on("ready", async () => {
    ...
    autoUpdater.on("update-downloaded", () => {
        dialog
            .showMessageBox(mainWindow, {
                ...
                detail:
                    "Restart now to apply the update, or continue and it will be applied next time you quit.",
    ...
    ipcMain.on(
        "manual-status-override",
        async (_e: any, payload: { newStatus: string }) => {
            ...
            try {
                if (newStatus === "NOT_STARTED") {
                    ...
                    if (currentJobTitle)
                        notificationManager?.onApplicationCompleted(
                            urlToComplete,
                            currentJobTitle,
                        );
                    hasStartedApplication = false;
    ...
    ipcMain.on("get-all-apps", (event: any) => {
        try {
            event.sender.send("all-apps-data", getAll());
        } catch {
    ...
    function exportApplicationData(): void {
        const rows = db
            .prepare("SELECT * FROM job_apps ORDER BY rowid DESC")
            .all() as any[];
        const csv = [
            "URL,Status,Applied At,Job Title,Company",
            ...rows.map(
                (r: any) =>
                    `"${r.url}",${r.status},"${r.applied_at}","${r.job_title || ""}","${r.company || ""}"`,
            ),
        ].join("\n");
    ...
    function isJobListingUrl(url: string): boolean {
        ...
        return [
            ...
            "dice.com/jobs",
            "wellfound.com/jobs",
            "joinhandshake.com/explore",
            "joinhandshake.com/job-search",
        ].some((p) => u.includes(p));
    }
    ...
    function hasLeftJobBoard(newUrl: string): boolean {
        ...
        if (
            newHost.includes(currentJobBoard) ||
            currentJobBoard.includes(newDomain)
        )
            return false;
    ...
    function extractJobTitle(bodyText: string, url: string): string {
        ...
        const handshakeMatch = url.match(
            /joinhandshake\.com\/(?:job-search|jobs)\/(\d+)/i,
        );
        ...
        try {
            ...
            if (last && !/^\d+$/.test(last))
                return last.replace(/[-_]/g, " ").substring(0, 50);
        } catch { }
        return "Job Position";
    }
    ...
    async function applyStatusChange(status: Status, url: string): Promise<void> {
        if (status === "NOT_STARTED") {
            ...
            if (!hasStartedApplication) {
                ...
                sendNativeNotification(
                    "HiredFinally",
                    `Application started: ${currentJobTitle || url}`,
                );
                runExtractionWorker();
            } else {
            ...
            if (urlToComplete) {
                ...
                sendNativeNotification(
                    "HiredFinally",
                    `Application submitted: ${currentJobTitle || urlToComplete}`,
                );
            }
            hasStartedApplication = false;
            lastNotStartedURL = "";
        ...
        for (const row of pending) {
            ...
            try {
                ...
                if (meta) {
                    ...
                    const still = (getPendingExtraction() as PendingRow[]).find(
                        (r: PendingRow) => r.url === row.url,
                    );
    ...
    async function processPageStatus(signals: PageSignals): Promise<void> {
        try {
            ...
            if (
                !isIndeedApplyFlow &&
                hasLeftJobBoard(url) &&
                newDomainForReset !== lastResetDomain
            ) {
            ...
            const isCompletionSignal =
                effectiveStatus === "COMPLETED" && signals.hasCompletionToast === true;
